...
import { Shield, Moon, Sun, Save, LogOut, User as UserIcon, LogIn, Lightbulb } from 'lucide-react';
      ...
      <nav className="sticky top-0 z-40 w-full backdrop-blur-md bg-white/80 dark:bg-slate-900/80 border-b border-slate-200 dark:border-slate-800 transition-colors duration-300">
        <div className="container mx-auto px-4 h-16 flex items-center justify-between">
          ...
          <div className="flex items-center gap-2 sm:gap-4">
            ...
            {/* Suggestion Box Button (Available to all) */}
            <button
              onClick={() => setShowSuggestionModal(true)}
              className="p-2 text-slate-500 hover:text-amber-500 dark:text-slate-400 dark:hover:text-amber-400 rounded-full hover:bg-slate-100 dark:hover:bg-slate-800 transition-colors"
              title={t('tooltip_suggestion')}
            >
